...
- 在运行命令时，注意输出正确格式的调用命令。 你的默认工作目录中有空格！ 始终确认使用正确的引号包裹了命令参数。
- 不要通过直接修改package.json再同步的模式安装依赖，始终通过运行pnpm add命令的方式，这条也适用于其他语言的包管理器，例如 uv, cargo 或是 nuget 。你通常应当安装最新稳定版，不要直接安装你记忆里的版本号。
- 你可以自由的建立git分支，但是，除非用户显式提出，不要主动建立工作树。
- 跳过 Skill 中关于你在doc目录下创建spec和plan的要求。不要将这些文件写在目录中，提交到git里。简单将其输出到上下文中即可。使用提问工具向我提问。
